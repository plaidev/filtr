...
_centerSphere = function(point, origin) {
  return __distance(point, origin);
}

/**
 * @param p {Array} - [<longitude>, <latitude>]
 */
__isGeographyPoint = function(p) {
...
/**
 * @param r {Array} - [[<latitude>, <longitude>], <radius>]
 */
__isGeographyCircleRegion = function(r) {
  if (!Array.isArray(r) || r.length < 2) {
    return false;
  }
  var origin = r[0]
    , radius = r[1];
  if (!__isGeographyPoint(origin) || 'number' != typeof radius) {
    return false;
  }
  return true;
}
...
/**
 * @param p1 {Array} - [<latitude>, <longitude>]
 * @param p2 {Array} - [<latitude>, <longitude>]
 */
__distance = function(p1, p2) {
  ...
  var d = Math.round(
    Math.acos(
      ...
      Math.cos(lat2) * Math.cos(lat1) * Math.cos(lng1 - lng2)
    ) * R  
  );
  return d;
}
...
Filtr.comparators = {
  ...
  , $geoWithin: function (p, test) {
    if (!__isGeographyPoint(p)) {
      return false;
    }
    for (var op in test) {
      if (op === '$centerSphere') {
        var region = test[op];
        if (!__isGeographyCircleRegion(region)) {
          return false;
        }

        var origin = region[0],
            radius = region[1];
        return _centerSphere(p, origin) <= radius;
      }
    }
    return false;
  }
